...
namespace Tiny
{
    ...
    TinyTexture* TinyTextureManager::load(const std::string& fileName, bool genMipmap)
    {
        uint8* data;
        uint32 length;
        getFileData(fileName, &data, length);
        TinyTexture* texture = TINY_NEW TinyTexture();
        texture->load(data, length, genMipmap);
        free(data);
        return texture;
    }
    
    TinyTexture* TinyTextureManager::load(const std::vector<std::string>& fileNames, bool genMipmap)
    {
        ...
        TinyTexture* texture = TINY_NEW TinyTexture();
        texture->load(datas, lengths, genMipmap);
        for (int i = 0; i < length; i ++)
        {
            free(datas[i]);
        }
